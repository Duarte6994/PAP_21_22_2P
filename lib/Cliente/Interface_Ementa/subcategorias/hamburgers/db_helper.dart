...
class DBHelper {
  ...
  _onCreate (Database db , int version )async{
    await db
        .execute('CREATE TABLE cart1 (id INTEGER PRIMARY KEY , productId VARCHAR UNIQUE,productName TEXT,initialPrice INTEGER, productPrice INTEGER , quantity INTEGER, unitTag TEXT , image TEXT )');
  }

  Future<Cart> insert(Cart cart)async{
    print(cart.toMap());
    var dbClient = await db ;
    await dbClient!.insert('cart1', cart.toMap());
    return cart ;
  }
